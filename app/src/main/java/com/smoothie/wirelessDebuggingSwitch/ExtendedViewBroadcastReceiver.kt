...
class ExtendedViewBroadcastReceiver : BroadcastReceiver() {
    ...
    companion object {
        ...
        const val ACTION_INSTANT_CONNECTION = "com.smoothie.wadbs.oneui_qs_action.INSTANT_CONNECTION"
    }
    ...
    override fun onReceive(context: Context?, intent: Intent?) {
        ...
        val wirelessDebugging = AdbWifi.getPrivilegeMethod(context) ?: return
        when (action) {
            ACTION_COPY_DATA -> {
                wirelessDebugging.copyConnectionData()
                message = context.getString(R.string.message_copied)
                Toast.makeText(context, message, Toast.LENGTH_SHORT).show()
            }

            ACTION_INSTANT_CONNECTION -> {
                wirelessDebugging.syncConnectionData()
            }
        }
    }
}
